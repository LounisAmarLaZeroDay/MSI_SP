...
{
    class TableProductsViewModel : Bases.ViewModelBase
    {
        public TableProductsViewModel()
        {
            Search();
            initDelegateCommand();
            initComboBoxOrderBy();
        ...
        private void execute<Object>(Object obj)
        {
            ...
            switch (st)
            {
                ...
                case "Edit":
                    {
                        VisibilityTableEdit_Visible();
                        if (DataGridSelectedItem != null) TableProductsEditViewModel.Send(DataGridSelectedItem);
                    }
                    break;
                case "Delete":
                    {
        ...
        public void Search()
        {
            try
            {
                DataGridItemsSource = T_Helper.product.search(SearchValue, ref PageThis, ComboBoxOrderBySelected).ToList();
                Label_Page = T_Helper.product.GetLableSearch();
            }
            catch { }
        }
        ...
        {
            string[] names = typeof(PRODUCT).GetProperties().Select(property => property.Name).ToArray();
