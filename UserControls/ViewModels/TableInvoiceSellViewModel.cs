...
{
    class TableInvoiceSellViewModel : Bases.ViewModelBase
    {
        public TableInvoiceSellViewModel()
        {
            initDelegateCommand();
            DateBegin = T_Helper.InvoiceSell.GetBeginDate();
            DateEnd = T_Helper.InvoiceSell.GetEndDate().AddDays(1);
            initEventHandler();
            initComboBoxOrderBy();
            Search();
        }
        ...
        public void Search()
        {
            try
            {
                DataGridItemsSource = T_Helper.InvoiceSell.search(SearchValue, ref PageThis, ComboBoxOrderBySelected, T_Helper.user.thisUser.ID, -1, ((DateTime)DateBegin), (DateTime)DateEnd).ToList();
                //DataGridItemsSource = T_Helper.InvoiceSell.search(SearchValue, ref PageThis, ComboBoxOrderBySelected, -1, -1, ((DateTime)DateBegin), (DateTime)DateEnd).ToList();
                Label_Page = T_Helper.InvoiceSell.GetLableSearch();
            }
            catch (Exception e)
            {
                F_File.LogError(e);
        ...
        {
            string[] names = typeof(INVOICES_SELLS).GetProperties().Select(property => property.Name).ToArray();
