...
namespace Data.Utils
{
    public static partial class T_Helper
    {
        public static class wholesaler
        {
            ...
            public static IQueryable<WHOLESALER> search(string p_srting, ref int PageThis, string p_orderBy)
            {
                if (p_srting == null) p_srting = "";
                IQueryable<WHOLESALER> _Query;
                try
                {
                    try
                    {
                        _Query = null;
                        _Query = _db.WHOLESALERS.Where(c => c.NAME.ToLower().Contains(p_srting) || c.DESCRIPTION.ToLower().Contains(p_srting)).OrderByDescending(p_orderBy);
                    ...
                    {
                        _Query = null;
                        _Query = _db.WHOLESALERS.Where(c => c.NAME.ToLower().Contains(p_srting) || c.DESCRIPTION.ToLower().Contains(p_srting));
                        return _Query;
                    }
            ...
            public static string SkipTake(ref int PageThis, ref IQueryable<WHOLESALER> p_Query)
            {
                GetPageSize();
            ...
            {
                return (_db.WHOLESALERS.Count() > 0) ? _db.WHOLESALERS.Max(u => u.ID) + 1 : 1;
            }
            public static WHOLESALER Get(int p_id)
            {
                return _db.WHOLESALERS.Single(c => c.ID == p_id);
            }

            public static void Delete(int p_id)
            {
                _db.WHOLESALERS.Remove(_db.WHOLESALERS.Single(c => c.ID == p_id));
                _db.SaveChanges();
            }
            //************************************************************************
            ...
            public static void EditFromObject(WHOLESALER p)
            {
            ...
            {
                return _db.WHOLESALERS.Any(o => o.CODE == p_string);
            }
            private static bool IsCanEditCode(int id, string p_string)
            {
                return !_db.WHOLESALERS.Any(o => o.CODE == p_string && o.ID != id);
            }
            private static bool IsExistName(string p_string)
            {
                return _db.WHOLESALERS.Any(o => o.NAME == p_string);
            }
            private static bool IsCanEditName(int id, string p_string)
            {
                return !_db.WHOLESALERS.Any(o => o.NAME == p_string && o.ID != id);
            }
            //*****************************************
        }
    }
